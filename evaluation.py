...
# Add this function to evaluate the posture
def evaluate_posture(keypoints_with_scores, threshold=0.1):
    ...
    if is_valid(left_shoulder) and is_valid(right_shoulder) and is_valid(left_hip) and is_valid(right_hip):
        # Calculate the angles or other criteria to evaluate posture
        # For simplicity, we check if the shoulders and hips are roughly horizontal
...
def draw_prediction_on_image(image, keypoints_with_scores):
    ...
    # Add posture evaluation result
    posture_result = evaluate_posture(keypoints_with_scores)
    ax.text(0.5, 0.05, posture_result, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes, color='red', fontsize=20)
